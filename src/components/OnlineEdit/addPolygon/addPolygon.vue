...
export default {
  ...
  methods: {
    ...
    // 切换模式
    changeModle(perPositionHeight, classificationType) {
      if (window.editHandler) {
        if (
          window.editHandler._editObject &&
          window.editHandler._editObject.id.indexOf("polygon-symbol") === 0
    ...
    drawModle(val) {
      switch (val) {
        ...
        case "1":
          ...
          this.changeModle(false, Cesium.ClassificationType.TERRAIN);
          break;
        case "2":
          this.perPositionHeight = undefined;
          this.classificationType = Cesium.ClassificationType.S3M_TILE;
          this.changeModle(undefined, Cesium.ClassificationType.S3M_TILE);
          break;
        default:
          this.perPositionHeight = true;
          this.classificationType = undefined;
