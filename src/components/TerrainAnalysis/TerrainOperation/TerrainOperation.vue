<template>
  <div v-show="terrainOperation">
    <div class="sm-function-module-content">
      ...
      <label class="sm-function-module-sub-section-setting">{{Resource.TilesetEditor}}</label>
      <div class="boxchild">
        <button @click="modifyTerrain" class="tbtn tbn1" type="button">{{Resource.modify}}</button>
        <button @click="clearModify" class="tbtn" type="button">{{Resource.clear}}</button>
      </div>
      <br />
      <div class="sm-point"></div>
      <label class="sm-function-module-sub-section-setting">{{Resource.TerrainMining}}</label>
      ...
      <div class="editBox">
        <label style="margin-left:10px">{{Resource.edit}}</label>
        <input style="margin-left: 5px" type="checkbox" v-model="isEdit" />&nbsp;&nbsp;
        <label>{{Resource.editZ}}</label>
        <input style="margin-left: 5px" type="checkbox" v-model="isEditZ" />
      </div>
      <div class="boxchild">
        <button @click="dig" @touchstart="dig" class="tbtn tbn1" type="button">{{Resource.Excavation}}</button>
        <button @click="clearDig" class="tbtn" type="button">{{Resource.clear}}</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    cType.push("terrain");
    cName.push(Resource.TerrainOperation);
  },
  methods: {
    // 挖掘模块
    dig() {
