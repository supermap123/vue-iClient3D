<template>
  <div v-show="GeologicComb">
    <div class="sm-function-module-content">
      ...
      <div class="sm-function-module-sub-section">
        <div>
          <label class="sm-function-module-sub-section-caption">{{Resource.TerrainMiningDepth}}</label>
          <input
            v-model="depth"
            type="number"
        ...
        <div>
          <label class="sm-function-module-sub-section-caption">{{Resource.StretchingHeightZDirection}}</label>
          <input
      ...
      <div>
        <button type="button" class="sm-btn sm-GeologicBodyOperation-btn" v-on:click="drawline">{{Resource.DrawLine}}</button>
        <button type="button" class="sm-btn sm-GeologicBodyOperation-btn" v-on:click="profile">{{Resource.Cut}}</button>
        <button type="button" class="sm-btn sm-GeologicBodyOperation-btn" v-on:click="addhole">{{Resource.Drilling}}</button>
        <button type="button" class="sm-btn sm-GeologicBodyOperation-btn" v-on:click="clip">{{Resource.clip}}</button>
        <button type="button" class="sm-btn sm-GeologicBodyOperation-btn" v-on:click="dlg">{{Resource.Excavation}}</button>
        <button type="button" class="sm-btn sm-GeologicBodyOperation-btn" @click="clear">{{Resource.clear}}</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    cType.push("analysis");
    cName.push(Resource.GeologicalBodyAnalysis);
  },
  mounted() {
      ...
      if (window.viewer) {
        ...
        handlerPolygon.movingEvt.addEventListener(function (windowPosition) {
          if (handlerPolygon.isDrawing) {
            tooltip.showAt(
              windowPosition,
              `<p>${Resource.ClickStartDrawing}</p><p>${Resource.RightClickEndDrawing}</p>`
            );
          } else {
            tooltip.showAt(windowPosition, `<p>${Resource.ClickDrawFirstPoint}</p>`);
          }
        });
        ...
        handlerPoint.movingEvt.addEventListener(function (windowPosition) {
          tooltip.showAt(windowPosition, `<p>${Resource.ClickDrawMultiplePointsAndRightClickEnd}</p>`);
        });
        ...
        handlerLine.movingEvt.addEventListener(function (windowPosition) {
          if (handlerLine.isDrawing) {
            tooltip.showAt(windowPosition, `<p>${Resource.DrawTwoPointsAsProfile}</p>`);
          } else {
            tooltip.showAt(windowPosition, `<p>${Resource.ClickDrawFirstPoint}</p>`);
          }
        });
