  ...
  methods: {
    ...
    analysis() {
      ...
      sightLineHandler.movingEvt.addEventListener((windowPosition) => {
        ...
        window.tooltip.showAt(windowPosition, `<p>${Resource.ClickLeftAddObservationPoint}</p>`);
        if (sightLineHandler.isDrawing) {
          window.tooltip.showAt(
            windowPosition,
            `<p>${Resource.ClickLeftAddObMultiplePoints}</p><p>${Resource.RightClickEndDrawing}</p>`
          );
        }
      });
      let that = this;
